...
// ---------- Rate limit: 요청 간 최소 지연 + 429 시 지수 백오프 재시도 ----------
const MIN_REQUEST_DELAY_MS = 150;
const RATE_LIMIT_MAX_RETRIES = 3;
const RATE_LIMIT_BACKOFF_BASE_MS = 1000;

let lastRequestTime = 0;
let rateLimitAlertCallback = null;
/** 429 발생 시 서버에 1분 일시 중단 요청용 (ms 전달). server에서 state.emergencyPauseUntil 설정 */
      ...
      try {
        ...
        if (is429 && attempt < RATE_LIMIT_MAX_RETRIES) {
          if (rateLimitAlertCallback) {
            try {
              rateLimitAlertCallback('API 요청 제한으로 재시도 중');
            } catch (_) {}
          }
          if (emergencyPauseCallback) {
            try {
              emergencyPauseCallback(60000);
            } catch (_) {}
          }
          const backoffMs = RATE_LIMIT_BACKOFF_BASE_MS * Math.pow(2, attempt);
          console.warn(`[Upbit] 429 too_many_requests — ${backoffMs}ms 후 재시도 (${attempt + 1}/${RATE_LIMIT_MAX_RETRIES})`);
