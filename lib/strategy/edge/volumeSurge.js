...
function compute(state, market) {
  ...
  let recentVolume = 0;
  const currentBucket = buckets.find(b => b.bucketKey === currentKey);
  if (currentBucket && typeof currentBucket.vol === 'number' && Number.isFinite(currentBucket.vol)) {
    recentVolume = currentBucket.vol;
  }
...
function check(state, market, symbol, mode = 'observe_only', threshold) {
  ...
  if (fallback) {
    return {
      ...
      avgBucketVolume,
      surgeValue,
      context: { ...context, reason: 'NEUTRAL_FALLBACK' }
    };
  }
