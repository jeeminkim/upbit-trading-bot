...
function evaluate(inputs, options = {}) {
  ...
  const fallbackUsed = [
    raw.signalScore == null || Number.isNaN(Number(raw.signalScore)),
    raw.regimeScore == null || Number.isNaN(Number(raw.regimeScore)),
    raw.volatilityFactor == null || Number.isNaN(Number(raw.volatilityFactor)),
    raw.liquidityFactor == null || Number.isNaN(Number(raw.liquidityFactor)),
    raw.slippageRiskInverse == null || Number.isNaN(Number(raw.slippageRiskInverse))
  ].some(Boolean);
  ...
  const details = {
    ...
    fallbackUsed,
    fallbackValue: fallbackUsed ? 'factor_clamped_to_0' : undefined
  };
