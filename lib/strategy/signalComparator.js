...
const edgeMetrics = require('./edge/edgeMetrics');
...
function normalizeStrategyScore(strategy, rawScore) {
  ...
  const scale = clampVal > 0 ? 0.5 / clampVal : 0;

  const arr = strategyScoreHistory[strategy];
  if (!Array.isArray(arr)) return rawScore;
  const safeRaw = Math.max(0, Math.min(1, Number(rawScore) || 0));
  arr.push(safeRaw);
  while (arr.length > MAX_SCORE_HISTORY) arr.shift();

  if (arr.length < minSamples) {
    edgeMetrics.incrementNormalizationFallback(strategy);
    try { console.debug(LOG_TAG_NORM, 'normalization fallback', { strategy, reason: 'MIN_SAMPLES', rawScore: safeRaw, sampleCount: arr.length }); } catch (_) {}
    return safeRaw;
  }
  ...
  if (std < epsilon) {
    edgeMetrics.incrementNormalizationFallback(strategy);
    try { console.debug(LOG_TAG_NORM, 'normalization fallback', { strategy, reason: 'STD_EPSILON', rawScore: safeRaw, std }); } catch (_) {}
    return safeRaw;
  }

  const z = (safeRaw - mean) / std;
  const zClamped = Math.max(-clampVal, Math.min(clampVal, z));
  const normalized = Math.max(0, Math.min(1, 0.5 + zClamped * scale));
  try { console.debug(LOG_TAG_NORM, 'normalized', { strategy, rawScore: safeRaw, normalizedScore: normalized, mean, std }); } catch (_) {}
  return normalized;
}
