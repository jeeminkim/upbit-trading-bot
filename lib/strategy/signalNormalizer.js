 ...
 * - B안: config.default.ORCHESTRATOR.normalizer_floor_score 설정 시 통과 신호에 최소 점수 부여
 */

const { createEmptySignal, clampScore } = require('./signalSchema');

const configDefault = require('../../config.default');
const SCORE_MAX_ENTRY = 10; // scalp entry_score_min ~ 10 구간 매핑
...
function normalizeScalpRaw(market, pipeline, scalpStateEntry, profile, p0Allowed) {
  ...
  const floorScore = (configDefault.ORCHESTRATOR && configDefault.ORCHESTRATOR.normalizer_floor_score != null)
    ? Number(configDefault.ORCHESTRATOR.normalizer_floor_score)
    : null;

  if (floorScore != null && !Number.isNaN(floorScore) && floorScore >= 0 && floorScore < 1) {
    base.score = clampScore(floorScore + ((scoreRaw - entryScoreMin) / (SCORE_MAX_ENTRY - entryScoreMin)) * (1 - floorScore));
  } else {
    base.score = clampScore((scoreRaw - entryScoreMin) / (SCORE_MAX_ENTRY - entryScoreMin));
  }
