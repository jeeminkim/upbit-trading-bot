...
/** 에러 코드 402/403, 메시지 키워드: billing, quota, limit exceeded, inactive → 결제/할당량 오류로 간주 */
function isBillingOrQuotaError(error) {
  if (!error) return false;
  const msg = (error?.message || '').toLowerCase();
  const code = (error?.code || error?.status || '').toString();
  const status = error?.response?.status;
  const billingKeywords = /billing|quota|limit\s*exceeded|inactive|결제\s*비활성|할당량\s*초과|리소스\s*소진|resource_exhausted/;
  if (billingKeywords.test(msg)) return true;
  if (code === '402' || code === '403' || status === 402 || status === 403) {
    if (billingKeywords.test(msg)) return true;
    if (/payment|billing|quota|disabled|리소스/i.test(msg)) return true;
    const body = (error?.response?.data && typeof error.response.data === 'object')
      ? JSON.stringify(error.response.data) : '';
    if (billingKeywords.test(body) || /payment|billing|quota|disabled|리소스/i.test(body)) return true;
  }
  return false;
}

/** 호출부 catch에서 사용: 결제/할당량 에러면 콜백 호출(EngineStateStore.update 등). 호출 후 기본값 반환하도록 사용 */
function handleGeminiError(error) {
  if (!error) return;
  if (isBillingOrQuotaError(error)) {
    console.warn('[Gemini] 결제/할당량 에러 포착 — AI 비활성화 콜백 호출:', error?.message);
...
/** 거래 부재 원인 진단: 구조화된 진단 입력을 받아 3줄 요약 (역할 B 통일용) */
async function askGeminiForNoTradeDiagnosis(structuredDiagnosisText) {
  if (!genAI || !GEMINI_API_KEY || !structuredDiagnosisText) return null;
  const prompt = `아래는 업비트 스캘핑 봇의 최근 12시간 매매·거절·상태를 구조화한 진단 입력입니다. 전문가 관점에서 **왜 거래가 체결되지 않았는지** 진단해줘. 답변은 반드시 3줄 요약으로만 제한해. (예: "현재 RSI가 설정값보다 높아 진입 대기 중", "잔고 부족으로 주문 취소" 등 구체적 이유)\n\n[구조화 진단 입력]\n${structuredDiagnosisText}`;
  try {
    const text = await getGeminiResponse(prompt);
    if (!text || text === GEMINI_DELAY_MESSAGE) return null;
    const trimmed = (text || '').trim();
    if (trimmed.length < 10) return null;
    return trimmed.length > 500 ? trimmed.slice(0, 497) + '…' : trimmed;
  } catch (e) {
    if (isBillingOrQuotaError(e) && onBillingDisabledCallback) {
      try { onBillingDisabledCallback(); } catch (_) {}
    }
    return null;
  }
...
/** 매매 로직 수정안 제안: 원칙 + 누적 진단을 바탕으로 보수적 수정안 20줄 이내 (역할 B 통일용) */
async function askGeminiForLogicSuggestion(principleAndAccumulatedText) {
  if (!genAI || !GEMINI_API_KEY || !principleAndAccumulatedText) return null;
  const prompt = `아래는 [안전 원칙]과 그동안 수집된 "거래 부재 원인" 진단 요약입니다. **원칙을 반드시 지키며** 매매 성공률을 높이기 위한 scalpEngine.js 또는 전략 수정안을 20줄 이내로 제안만 출력해줘. 안전장치 완화는 매우 보수적으로, 한 번에 1~2개만 제안하고, 근거 없는 threshold 완화는 금지. 수정 금지 항목이 있으면 함께 적어줘.\n\n${principleAndAccumulatedText}`;
  try {
    const text = await getGeminiResponse(prompt);
    if (!text || text === GEMINI_DELAY_MESSAGE) return null;
    const trimmed = (text || '').trim();
    if (trimmed.length < 10) return null;
    return trimmed.length > 2500 ? trimmed.slice(0, 2497) + '…' : trimmed;
  } catch (e) {
    if (isBillingOrQuotaError(e) && onBillingDisabledCallback) {
      try { onBillingDisabledCallback(); } catch (_) {}
    }
