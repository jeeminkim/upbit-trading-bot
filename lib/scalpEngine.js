...
const { getBreakEvenPrice, calculateNetProfitPct } = require(path.join(__dirname, '../src/shared/utils/math'));
const { UPBIT_FEE_RATE, MIN_PROFIT_EXIT_PCT } = require(path.join(__dirname, '../src/shared/constants'));
...
function shouldExitScalp(position, snapshot, currentPrice, currentEntryScore) {
  ...
  const netReturnPct = calculateNetProfitPct(entryPrice, currentPrice, UPBIT_FEE_RATE);
  const breakEvenPrice = getBreakEvenPrice(entryPrice, UPBIT_FEE_RATE);
  const aboveBreakEven = currentPrice >= breakEvenPrice;
  ...
  const minExitPct = Math.max(targetPct != null ? targetPct : 0, MIN_PROFIT_EXIT_PCT);
  if (targetPct != null && targetPct > 0 && aboveBreakEven && netReturnPct >= minExitPct) {
  ...
  if (trailingPct != null && trailingPct > 0 && highSinceEntry > 0 && aboveBreakEven) {
    const dropFromHighPct = (highSinceEntry - currentPrice) / highSinceEntry * 100;
    if (dropFromHighPct >= trailingPct && netReturnPct >= MIN_PROFIT_EXIT_PCT) {
      return { exit: true, reason: 'trailing_stop' };
    }
  }

  const scoreOut = profile.score_out_threshold;
  if (scoreOut != null && currentEntryScore != null && !Number.isNaN(Number(currentEntryScore)) && Number(currentEntryScore) <= scoreOut && aboveBreakEven && netReturnPct >= MIN_PROFIT_EXIT_PCT) {
    return { exit: true, reason: 'score_out' };
  }

  if (aboveBreakEven && netReturnPct >= Math.max(profile.min_take_profit_floor_pct || 0, MIN_PROFIT_EXIT_PCT)) {
...
function runEntryPipeline(snapshot, prevHigh, currentPrice, market, marketContext = null, availableKrw = null) {
  ...
  if (prevHigh != null && currentPrice != null && bufferTicks != null && tickSize != null) {
    ...
    let breakoutFactor = 1.0;
    try {
      const edgeCfg = configDefault.EDGE_LAYER;
      if (edgeCfg && edgeCfg.breakoutFactorByAsset && edgeCfg.breakoutFactorByAsset[symbol] != null) {
        breakoutFactor = Number(edgeCfg.breakoutFactorByAsset[symbol]) || 1.0;
      }
    } catch (_) {}
    const spreadBuffer = bufferTicks * tickSize * breakoutFactor;
    const threshold = prevHigh + spreadBuffer;
...
/**
 * 매수 주문에 사용할 금액(KRW) 결정 — 경주마 모드·실시간 가동 시간대·조건부 티어(FULL_50/MEDIUM_25/LIGHT_10/NORMAL/BLOCKED)
 * - BLOCKED 시 매수 금지(amountKrw 0). LIGHT_10=10%, NORMAL=기본 엔진 금액.
 * @param {{ orderableKrw: number, totalCoinEval: number, isRaceHorseMode: boolean, isRaceHorseTimeWindow: boolean, raceHorseTier?: string, minOrderKrw?: number, symbol?: string }} opts
 * @returns {{ amountKrw: number, skipReason?: string, raceHorseTier?: string }}
 */
function getBuyOrderAmountKrw(opts) {
  ...
  const raceHorseTier = opts?.raceHorseTier || null;
  ...
  const inRaceHorseWindow = isRaceHorseMode && isRaceHorseTimeWindow;
  const useRaceHorseFraction = inRaceHorseWindow && (raceHorseTier === 'FULL_50' || raceHorseTier === 'MEDIUM_25' || raceHorseTier === 'LIGHT_10');
  const ratio = raceHorseTier === 'FULL_50' ? 0.5 : raceHorseTier === 'MEDIUM_25' ? 0.25 : raceHorseTier === 'LIGHT_10' ? 0.1 : null;
  if (inRaceHorseWindow && raceHorseTier === 'BLOCKED') {
    return { amountKrw: 0, skipReason: 'RACE_HORSE_BLOCKED', raceHorseTier: 'BLOCKED' };
  }

  if (!useRaceHorseFraction || ratio == null) {
    const profile = getProfile();
    let baseKrw = profile.min_order_krw != null ? profile.min_order_krw : 10000;
    const edgeCfg = configDefault.EDGE_LAYER;
    if (edgeCfg && edgeCfg.enabled && symbol) {
      try {
        const totalKrw = orderableKrw + totalCoinEval;
        const alloc = (edgeCfg.allocationByAsset && edgeCfg.allocationByAsset[symbol] != null) ? edgeCfg.allocationByAsset[symbol] : 0.15;
        const mode = (edgeCfg.operatingMode || 'A_BALANCED').toUpperCase();
        const mult = (edgeCfg.modeMultiplier && edgeCfg.modeMultiplier[mode] != null) ? edgeCfg.modeMultiplier[mode] : 1.0;
        const maxAgg = (edgeCfg.aggressiveScalpMaxAllocation != null) ? edgeCfg.aggressiveScalpMaxAllocation : 0.06;
        let cap = totalKrw * alloc * mult;
        cap = Math.min(cap, totalKrw * maxAgg);
        baseKrw = Math.min(baseKrw, Math.max(minOrderKrw, Math.min(orderableKrw, Math.floor(cap))));
      } catch (_) {}
    }
    ...
    return { amountKrw, raceHorseTier: raceHorseTier || null };
  }

  const total = orderableKrw + totalCoinEval;
  let amountKrw = Math.floor(total * ratio);
  amountKrw = Math.min(orderableKrw, amountKrw);
  if (amountKrw < minOrderKrw) {
    return { amountKrw: 0, skipReason: 'MIN_ORDER_KRW', raceHorseTier };
  }
  return { amountKrw, raceHorseTier };
}
