...
function shouldExitScalp(position, snapshot, currentPrice, currentEntryScore) {
  ...
  const holdSec = (Date.now() - position.entryTimeMs) / 1000;
...
/** 진입 최소 스코어: 승인된 대형주는 2 낮춰 더 잦은 매매 유도 */
function getEffectiveEntryScoreMin(baseMin, market) {
  const sym = (market || '').replace(/^KRW-/, '').toUpperCase();
  if (!getAggressiveSymbols().includes(sym)) return baseMin;
  return Math.max(1, (baseMin != null ? baseMin : 4) - 2);
}
