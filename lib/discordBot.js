...
async function handleInteraction(interaction) {
  ...
  if (!adminGuard.isAdminUser(userId, effectiveAdminId)) {
    console.warn('[MyScalpBot] Auth Failed — User ID:', userId);
  ...
  try {
    ...
    if (customId === 'ai_analysis' && handlers?.aiAutoAnalysis) {
      await interaction.editReply({ content: '상위 5종목 수집 중… (RSI·체결강도·5분봉) → Gemini 분석 중 🧠' }).catch(() => {});
      const raw = await handlers.aiAutoAnalysis();
    ...
    if (customId === 'daily_log_analysis' && handlers?.analyst?.dailyLogAnalysis) {
      await interaction.editReply({ content: '오늘자 로그 수집·분석 중… (일 단위)' }).catch(() => {});
      const result = await handlers.analyst.dailyLogAnalysis();
      const content = (result && result.content) || '결과 없음';
      const MAX_LEN = 2000;
      const chunks = content.length <= MAX_LEN ? [content] : content.match(new RegExp(`.{1,${MAX_LEN}}`, 'gs')) || [content];
      await interaction.editReply({ content: chunks[0], embeds: [] }).catch(() => {});
    ...
    if (typeof customId === 'string' && customId.startsWith('analyst_')) {
      ...
      const embed = await handleAnalystButton(customId);
    ...
    if (customId === 'engine_start' && handlers?.engineStart) {
      ...
      const msg = result?.success ? '⏱️ 독립 스캘프 3시간 연장되었습니다.' : '연장 가능한 시점이 아닙니다. (1시간 미만일 때만 연장 가능)';
      await interaction.editReply({ content: msg }).catch(() => {});
    } else {
...
function buildControlRow() {
  ...
  const row3 = new MessageActionRow().addComponents(
    ...
    new MessageButton().setCustomId('independent_scalp_stop').setLabel('독립 스캘프 중지').setStyle('SECONDARY').setEmoji('🛑')
  );
...
async function start(opts) {
  const { token, channelId, adminId: envAdminId, tradingLogChannelId, aiAnalysisChannelId, handlers: h } = opts || {};
  if (!token || !channelId) {
    const missing = [];
  ...
  /** ready 시 지정 채널에 역할 A·역할 B·현재 상태 임베드를 한 번만 출력. 데이터 로딩 후 호출되므로 실패 시 재연결 시 재시도 */
...
/** 체결 알림: #trading-log 채널(또는 제어 채널)에 종목명·금액·수익률 즉시 보고 */
function sendTradeAlert(data) {
  const { ticker, side, price, quantity, currentReturnPct } = data || {};
  const sideKo = (side || '').toLowerCase() === 'buy' ? '매수' : '매도';
  const pctNum = currentReturnPct != null ? Number(currentReturnPct) : null;
  const emoji = pctNum != null ? (pctNum >= 0 ? '🟢' : '🔴') : '';
  const pctStr = pctNum != null ? `\n${emoji} 현재 수익률: ${(pctNum >= 0 ? '+' : '') + pctNum.toFixed(2)}%` : '';
  const embed = new MessageEmbed()
    .setColor(side === 'buy' ? 0x57f287 : 0xed4245)
    .setTitle(`체결 알림 — ${sideKo}`)
    .addFields(
      { name: '종목', value: ticker || '—', inline: true },
      { name: '체결가', value: price != null ? price.toLocaleString('ko-KR') + ' 원' : '—', inline: true },
      { name: '수량', value: quantity != null ? String(quantity) : '—', inline: true }
    )
    .setTimestamp();
  if (pctStr) embed.setDescription(pctStr.trim());
  const targetChannelId = tradingLogChannelIdStored || channelIdStored;
  if (targetChannelId && client) {
    return sendToChannelId(targetChannelId, embed);
  }
