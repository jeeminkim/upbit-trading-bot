...
async function handleInteraction(interaction) {
  ...
  try {
    ...
    if (customId === 'engine_start' && handlers?.engineStart) {
      ...
      const msg = (result && typeof result === 'object' && result.message) ? result.message : '자동매매 엔진이 시작되었습니다.';
      await interaction.editReply({ content: msg }).catch(() => {});
      if (result && result.success && !result.noop && msg) sendToChannel(msg);
    } else if (customId === 'engine_stop' && handlers?.engineStop) {
      const result = await handlers.engineStop();
      const msg = (result && typeof result === 'object' && result.message) ? result.message : '자동매매 엔진을 중지했습니다.';
      await interaction.editReply({ content: msg }).catch(() => {});
    ...
    if (!isMainReportSent && handlers?.getStartupStatusEmbed && controlChannel) {
      try {
        ...
        const fallback = new MessageEmbed()
          .setTitle('📊 현재 상태')
          .setColor(0x5865f2)
          .setDescription('자동매매 엔진 상태: 🔴 중지됨')
          .addFields(
            { name: '자동매매 엔진 상태', value: '🔴 중지됨', inline: true },
...
/** 종료 직전 Discord에 메시지 전송 (최대 4초 대기). 실패해도 무한 대기하지 않음. */
function sendShutdownMessage(content) {
  return Promise.race([
    sendToChannel(content),
