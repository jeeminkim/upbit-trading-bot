...
class ScalpState {
  constructor() {
    ...
    /** 재기동 후 복원했으나 자동 재가동하지 않음 — 사용자 수동 start 필요 */
    this.pausedAfterRestart = false;
    /** persistence에서 endTime 등 참고용으로 복원된 상태 (UI 표시용) */
  ...
  /** 서버 재기동 시 system_state.json 복구용. endTime(ms)만 참고 복원, 자동 재가동 안 함 (진입 불가) */
  restoreFromPersistence(endTime) {
    if (endTime == null || typeof endTime !== 'number') return;
    if (Date.now() >= endTime) return;
    this.endTime = endTime;
    this.pausedAfterRestart = true;
  ...
  /** 진입 가능 여부 (매수 엔진 ON) */
  canEnterNewTrades() {
    return this.isRunning && this.priorityOwner === 'SCALP' && !this.isRiskHalt();
  }

  /** 청산은 포지션/잔고가 있으면 항상 허용 (엔진 paused 여부와 무관) */
  canManageExits() {
    return true;
  }
}
