...
/** 독립 스캘프 전용 최소 주문 금액 (업비트 5000보다 여유 두어 qty=0·엣지 완화) */
const SCALP_MIN_ORDER_KRW = 6000;
const TAKE_PROFIT_PCT = 0.2;
const STOP_LOSS_PCT = -0.15;
/** 최소 거래 간격(초): 무한 매매 방지 */
const MIN_TRADE_INTERVAL_MS = 30 * 1000;
/** [SCALP_RISK] 동일 사유 로그 반복 방지: 이 시간(ms) 내 동일 reason 세트는 로그 1회만 */
const SCALP_RISK_LOG_COOLDOWN_MS = 30000;
/** 초 scalp 전용 심볼 cooldown(분). MAIN은 riskGate 기본 30분 유지 */
const SCALP_COOLDOWN_MINUTES = 10;
/** 잔액 부족 / SCALP_SKIP insufficient 로그·알림 쓰로틀: 틱당 1회가 아닌 60초 1회로 제한 */
const INSUFFICIENT_FUNDS_LOG_THROTTLE_MS = 60 * 1000;

/** SCALP_RISK 로그 쓰로틀링. scope/symbol이 다르면 별도 로그로 취급. 스킵 시 true, 로그 진행 시 false 반환 후 시간 갱신 */
const lastScalpRiskLogByKey = {};
function shouldSkipScalpRiskLog(reasons, scope, symbolOrAll) {
  const sortedReasons = (reasons || []).slice().sort().join(',');
  const key = `${scope || 'global'}:${symbolOrAll || 'ALL'}:${sortedReasons}`;
  const now = Date.now();
  const last = lastScalpRiskLogByKey[key];
  if (last != null && (now - last) < SCALP_RISK_LOG_COOLDOWN_MS) return true;
  lastScalpRiskLogByKey[key] = now;
  return false;
}

/** 잔액 부족 관련 로그/알림 60초 1회로 제한. 사용 후 true 반환 시 로그 진행, false면 스킵 */
let lastInsufficientFundsLogAt = 0;
function shouldLogInsufficientFunds() {
  const now = Date.now();
  if (now - lastInsufficientFundsLogAt < INSUFFICIENT_FUNDS_LOG_THROTTLE_MS) return false;
...
/** 주문 응답에서 유효 수량 추출. 비정상/무한대가 아니면 숫자, 아니면 0 */
function normalizeOrderVolume(order) {
  const raw = order?.executed_volume != null ? order.executed_volume : order?.volume;
...
async function tick(ctx) {
  ...
  // 전 엔진 공통 cash lock: 활성 시 신규 매수 중단 (청산은 tickExit에서 계속 허용)
  if (state?.cashLock?.active) return;

  // 틱 전체 skip: 주문 가능 금액이 스캘프 최소 미만이면 진입 시도·로그 반복 방지
  const orderableKrw = state?.assets?.orderableKrw ?? 0;
  if (orderableKrw < SCALP_MIN_ORDER_KRW) {
    if (shouldLogInsufficientFunds()) {
      logLine(`[SCALP_SKIP] reason=insufficient_funds_for_scalp orderableKrw=${orderableKrw} min=${SCALP_MIN_ORDER_KRW} (tick skipped)`);
  ...
  const gateCtx = {
    ...
    lastEntryBySymbol: ctx?.lastEntryBySymbol || {},
    recentTrades: state?.trades || []
  };

  const globalVerdict = riskGate.checkScalpLight(gateCtx, '', { duplicate_cooldown_minutes: SCALP_COOLDOWN_MINUTES });
  if (!globalVerdict.allowed) {
    if (!shouldSkipScalpRiskLog(globalVerdict.reasons, 'global', 'ALL')) {
      try {
  ...
  try {
    ...
    // await 이후 상태 재확인: stop 되었으면 진입 로직 중단
    if (!scalpState.isRunning || scalpState.priorityOwner !== 'SCALP') return;
    const tickerMap = {};
    ...
    for (const market of SCALP_MARKETS) {
      ...
      const scalpLightVerdict = riskGate.checkScalpLight(gateCtx, symbol, { duplicate_cooldown_minutes: SCALP_COOLDOWN_MINUTES });
      if (!scalpLightVerdict.allowed) {
        if (!shouldSkipScalpRiskLog(scalpLightVerdict.reasons, 'market', market)) {
          try {
            logLine('[SCALP_RISK] ' + JSON.stringify({ type: 'scalp_risk_gate', market, symbol, reasons: scalpLightVerdict.reasons, ts: Date.now() }));
          } catch (_) {}
        }
        continue;
      }
      ...
      // 독립 스캘프 전용: 총 자산의 50% 투입 (orderableKrw + 코인 평가액), 가용 KRW 초과 불가. 최소 주문 SCALP_MIN_ORDER_KRW
      const orderableKrw = state?.assets?.orderableKrw ?? 0;
      ...
      amountKrw = Math.max(SCALP_MIN_ORDER_KRW, amountKrw);
      if (amountKrw * (1 + UPBIT_FEE_RATE) > orderableKrw && orderableKrw > 0) {
        amountKrw = Math.floor(orderableKrw / (1 + UPBIT_FEE_RATE));
      }
      if (amountKrw < SCALP_MIN_ORDER_KRW) {
        if (shouldLogInsufficientFunds()) {
          logLine(`[SCALP_SKIP] symbol=${symbol} reason=insufficient_funds_for_scalp amountKrw=${amountKrw} min=${SCALP_MIN_ORDER_KRW} orderableKrw=${orderableKrw}`);
          if (ctx.sendAlert) ctx.sendAlert('⚠️ 잔액 부족으로 매수 건너뜀 (독립 스캘프)');
        }
        continue;
      }

      const finalOrderKrw = Math.round(amountKrw);
      if (!finalOrderKrw || finalOrderKrw < SCALP_MIN_ORDER_KRW) {
        logLine(`[SCALP_SKIP] symbol=${symbol} reason=final_order_below_min finalOrderKrw=${finalOrderKrw} min=${SCALP_MIN_ORDER_KRW}`);
        continue;
      }

      try {
        const order = await TradeExecutor.placeMarketBuyByPrice(apiKeys.accessKey, apiKeys.secretKey, market, finalOrderKrw, orderableKrw);
        const volume = normalizeOrderVolume(order);

        if (!volume || volume <= 0) {
          logLine(`[SCALP_ENTRY_WARN] symbol=${symbol} reason=invalid_volume volume=${volume} finalOrderKrw=${finalOrderKrw} orderId=${order?.uuid || 'n/a'}`);
          if (state && (state.analytics = state.analytics || {}) != null) {
            state.analytics.scalpInvalidVolumeCount = (state.analytics.scalpInvalidVolumeCount || 0) + 1;
          }
          continue;
        }
        ...
        scalpState.activePosition = {
          ...
          quantityKrw: finalOrderKrw,
          volume
        };
        scalpState.dailyEntries += 1;
        scalpState.lastEntryTime = Date.now();
        if (ctx.recordEntryForCooldown) ctx.recordEntryForCooldown(symbol);
        if (ctx.recordTrade) {
          ctx.recordTrade({
        ...
        logLine(`[SCALP_ENTRY] symbol=${symbol} side=BUY price=${scalpState.activePosition.entryPrice} qty=${volume} mode=TAKER amountKrw=${finalOrderKrw}`);
...
async function tickExit(ctx) {
  ...
  if (!pos) return;
  if (!pos.volume || pos.volume <= 0) {
    logLine(`[SCALP_POSITION_WARN] symbol=${pos?.symbol || 'unknown'} reason=invalid_position_volume volume=${pos?.volume}`);
    return;
  }
...
/** server.js에서 호출: 독립 스캘프 상태 (디스코드/대시보드용). globalState 전달 시 canEnterNewTrades에 cash lock 반영 */
function getStatus(globalState) {
  const engineCanEnter = scalpState.canEnterNewTrades();
  const cashLockActive = globalState?.cashLock?.active === true;
  return {
    ...
    pausedAfterRestart: scalpState.pausedAfterRestart,
    restoredFromPersistence: scalpState.restoredFromPersistence,
    canEnterNewTrades: engineCanEnter && !cashLockActive,
    canManageExits: scalpState.canManageExits()
  };
}
