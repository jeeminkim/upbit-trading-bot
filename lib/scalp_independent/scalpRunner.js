...
async function tick(ctx) {
  ...
  try {
    ...
    for (const market of SCALP_MARKETS) {
      ...
      if (amountKrw < MIN_ORDER_KRW) continue;

      try {
        const order = await TradeExecutor.placeMarketBuyByPrice(apiKeys.accessKey, apiKeys.secretKey, market, Math.round(amountKrw));
